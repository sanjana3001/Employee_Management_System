...
resource "aws_s3_bucket_object" "frontend_object" {
  ...
  bucket   = aws_s3_bucket.employee_bucket.bucket
...
resource "aws_s3_bucket_object" "frontend_config" {
  key     = "config.js"
  content = <<EOF
export const cognitoLoginUrl = "https://${aws_cognito_user_pool_domain.cog_domain.domain}.auth.${data.aws_region.current.name}.amazoncognito.com";
...
resource "aws_cloudfront_distribution" "distribution" {
  origin {
    domain_name = aws_s3_bucket.employee_bucket.bucket_regional_domain_name
    origin_id   = "s3"
...
resource "aws_s3_bucket_policy" "OAI_policy" {
  bucket = aws_s3_bucket.employee_bucket.id
...
data "aws_iam_policy_document" "s3_policy" {
  statement {
    ...
    resources = ["${aws_s3_bucket.employee_bucket.arn}/*"]
...
resource "aws_lambda_permission" "auto_confirm" {
  ...
  source_arn = aws_cognito_user_pool.cog_pool.arn
}

resource "aws_cognito_user_pool" "cog_pool" {
...
resource "aws_cognito_user_pool_domain" "cog_domain" {
  domain       = "test-${random_id.id.hex}"
  user_pool_id = aws_cognito_user_pool.cog_pool.id
}
